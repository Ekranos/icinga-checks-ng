...
struct Opts {
    ...
    #[clap(short, default_value = "127.0.0.1:53")]
    server_address: SocketAddr,
    /// The name to resolve
    #[clap(short = 'H', long)]
    hostname: Name,
    /// Choose how to connect to the DNS server (tcp, udp)
    #[clap(short = 't', long, default_value = "udp")]
...
fn check_dns_ng(opts: Opts) -> Result<Resource, Box<dyn Error>> {
    ...
    let mut ips = answers
        .iter()
        .filter_map(|a| a.data().map(|d| d.to_string()))
        .collect::<Vec<_>>();
